    ...
    "lessons": [
      ...
      },
      {
        ...
        "content": [
          {
            "type": "p",
            "text": "ここまでの小節では、ORM やマイグレーションを通して「データをどう保存するか」を学んできました。次に考えるべきなのは、「そのデータを外部からどう操作するか」です。"
          },
          {
            "type": "p",
            "text": "その役割を担うのが API です。この小節では、まず API とは何かを整理し、その上で RESTful 規約という設計ルールを見ていきます。"
          },
      
          {
            "type": "p",
            "text": "API（Application Programming Interface）とは、「アプリケーションがデータを操作するための入口」です。Web アプリケーションにおいては、API はデータベースそのものではなく、**データに対する操作だけを外部に公開する窓口**として機能します。"
          },
          {
            "type": "p",
            "text": "例えば、ユーザー情報を扱う場合、フロントエンドや他のサービスはデータベースに直接触ることはできません。その代わりに、API を通して次のような操作を行います。"
          },
          {
            "type": "ul",
            "items": [
          ...
          {
            "type": "p",
            "text": "このように、API は「データをどう見せるか」ではなく、「データをどう操作できるか」を定義するものです。画面（UI）とは役割が明確に分かれています。"
          },
      
          {
            "type": "p",
            "text": "ここで問題になるのが、「API をどんなルールで設計するか」です。API は自由に作れてしまうため、設計者ごとにバラバラな形になりがちです。そこで登場するのが RESTful 規約です。"
          },
          {
            "type": "p",
            "text": "RESTful 規約は、「API をどう設計すれば分かりやすく、一貫性のあるものになるか」を整理した考え方です。特に重要なのは、「API はデータ（リソース）を中心に考える」という点です。"
          },
          {
            "type": "p",
